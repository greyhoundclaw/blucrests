...
async function depositRoutes(req, res, body) {
    try {
        if (req.url === '/api/v1/deposits' && req.method === 'POST') {
            ...
            await notifications.createNotification({ user_id: req.user.id, title: 'Deposit request received', message: `Your ${method === 'GIFTCARD' ? 'gift card' : 'Bitcoin'} deposit to ${targetAccount?.account_kind === 'JOINT' ? 'the joint account' : 'your account'} is pending review.`, type: 'INFO', action_link: targetAccount?.account_kind === 'JOINT' ? '/joint-accounts' : '/deposit' });
