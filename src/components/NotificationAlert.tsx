...
export default function NotificationAlert({ count, onView }: { count: number; onView: () => void | Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-slate-950/65 backdrop-blur-sm flex items-center justify-center p-4" role="alertdialog" aria-modal="true" aria-label="New notification">
    <div className="w-full max-w-md rounded-[2rem] bg-white p-7 text-center shadow-2xl border border-blue-100">
      ...
      <button autoFocus disabled={opening} onClick={viewNotifications} className="w-full h-12 mt-6 rounded-xl bg-[#003399] text-white text-sm font-bold disabled:opacity-60 flex items-center justify-center gap-2">
        {opening && <LoaderCircle className="w-4 h-4 animate-spin" />}
        {opening ? 'Opening…' : 'View notifications'}
      </button>
    </div>
  </div>;
}
